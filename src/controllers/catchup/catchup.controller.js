...
const getAllCatchup = async (req, res, next) => {
    try {
        ...
        const catchup = await Catchup.find({owner: {$in:[...user?.following,...user?.followers]}}).populate("owner","firstName lastName avatar").populate({path:"referenceId",select:"company",populate:{path:"company",model:"Page",select:"name"}});
